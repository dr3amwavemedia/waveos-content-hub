-- Client workspaces provisioned by Dream Wave staff must not become personal
-- workspaces of the staff member who created them. Non-staff self-service
-- creation keeps its existing owner-membership behavior.
...
BEGIN
  ...
  _is_staff := public.is_dream_wave_staff(_uid);
  ...
  VALUES (_ws_id, _uid, 'workspace_created', 'workspace', _ws_id,
    jsonb_build_object('slug', _new_slug, 'self_service', NOT _is_staff));
