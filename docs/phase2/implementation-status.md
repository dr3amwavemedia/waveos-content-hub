...
- The local browser fixture verifies link/programmatic warnings, cancellation retaining text and confirmed navigation reaching its destination. An unstable fixture field label was corrected during regression testing. GitHub runs all six navigation cases; the final check result is recorded in PR #75. Native refresh/tab-close prompts and staged authenticated workflows remain outstanding.
- The Forward regression exposed incorrect cancelled-history rollback in the installed TanStack history package. npm overrides now pin its compatible 1.162.4 patch, which rolls back the actual navigation delta. The npm lockfile is synchronized with the existing manifest (including its previously omitted QR-code packages and pinned Playwright version). A clean, unchanged-lockfile `npm ci` passed; CI no longer regenerates the lockfile before installing. Build/typecheck/lint and unit results for this final dependency set are recorded in PR #75.
