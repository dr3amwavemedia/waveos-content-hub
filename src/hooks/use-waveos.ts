...
async function loadContext(): Promise<CurrentUserContext> {
  ...
  if (acting) {
    return {
      userId: acting.userId, email: acting.email, firstName: acting.firstName, lastName: acting.lastName,
      avatarUrl: null, isStaff: true, isDreamWaveOwner: false, staffType: acting.staffType ?? "sales",
      roles: ["dream_wave_team"], actingAsStaff: true, actualUserId: user.id,
    };
  }
...
function readActiveWorkspaceId() {
  return typeof window === "undefined" ? null : localStorage.getItem(ACTIVE_WORKSPACE_KEY);
}

export function useCurrentUser() {
  const impersonate = useImpersonateClient();
  const [activeWorkspaceId, setActiveWorkspaceId] = useState<string | null>(readActiveWorkspaceId);
  const query = useQuery({ queryKey: ["waveos", "current-user"], queryFn: loadContext, staleTime: 60_000 });

  useEffect(() => {
    const sync = () => setActiveWorkspaceId(readActiveWorkspaceId());
  ...
  return {
    ...query,
    data: { ...query.data, isStaff: false, isDreamWaveOwner: false, staffType: null },
  };
}
...
export function useWorkspaces() {
  ...
  const impersonate = useImpersonateClient();
  const previewWorkspaceId = typeof window !== "undefined" && impersonate.on ? localStorage.getItem(ACTIVE_WORKSPACE_KEY) : null;
  return useQuery({
