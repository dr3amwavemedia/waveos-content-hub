...
type PaymentInvoice = Pick<Database["public"]["Tables"]["client_invoices"]["Row"], "amount_cents" | "amount_paid_cents" | "currency" | "status" | "payment_plan" | "billing_month" | "checkout_payment_type" | "checkout_payment_cents">;
...
export function PaymentProgress({ invoice }: { invoice: PaymentInvoice }) {
  ...
  return <div className="my-3 space-y-2 rounded-lg bg-primary/5 p-3 text-sm">
    ...
    {received == null || total == null ? <p className="text-xs text-muted-foreground">Payment amounts have not been recorded yet.</p> : <>
      ...
      {dueNow > 0 && dueNow < Math.max(0, total - received) && <p className="font-medium text-primary">{money(dueNow)} due with the next payment</p>}
