...
export function AdminIdentityManager() {
  ...
  const renameStaff = useMutation({
    ...
    onError: (error: unknown) =>
      toast.error(error instanceof Error ? error.message : "Could not update staff name."),
  });
  ...
  const renameWorkspace = useMutation({
    ...
    onError: (error: unknown) =>
      toast.error(error instanceof Error ? error.message : "Could not update client name."),
  });
  ...
  const renameLead = useMutation({
    mutationFn: async () => {
      if (!leadEdit) return;
      const primary =
        leadEdit.crm_contacts?.find((contact) => contact.is_primary) ?? leadEdit.crm_contacts?.[0];
    ...
    onError: (error: unknown) =>
      toast.error(error instanceof Error ? error.message : "Could not update lead name."),
  });
    ...
    <div className="surface-card p-5">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Preview WaveOS as a staff member, or correct staff, client, and lead names without
            recreating accounts.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-3">
        <section className="rounded-xl border border-border bg-background/35 p-4">
          ...
          <div className="mt-3 space-y-2">
            ...
            {!staffQ.isLoading && !editableStaff.length && (
          ...
          <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
            {(leadsQ.data ?? []).map((lead) => (
              <button
                ...
                onClick={() => {
                  const primary =
                    lead.crm_contacts?.find((contact) => contact.is_primary) ??
                    lead.crm_contacts?.[0];
                  setLeadEdit(lead);
                  setWorkspaceEdit(null);
                ...
                className="flex w-full items-center justify-between rounded-lg border border-border/70 p-2 text-left hover:bg-elevated"
              >
                <span className="truncate text-sm">{lead.business_name}</span>
                <Pencil className="h-3.5 w-3.5 text-muted-foreground" />
              </button>
            ))}
          </div>
      ...
      {staffEdit && (
        <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-4">
          ...
          <button
            onClick={() => renameStaff.mutate()}
            disabled={renameStaff.isPending || !firstName.trim()}
      ...
      {(workspaceEdit || leadEdit) && (
        <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-primary">
            {workspaceEdit ? "Edit client name" : "Edit lead name"}
          </p>
          <input
            value={name}
          ...
          <button
            onClick={() => (workspaceEdit ? renameWorkspace.mutate() : renameLead.mutate())}
            disabled={!name.trim() || renameWorkspace.isPending || renameLead.isPending}
            className="mt-3 inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground disabled:opacity-50"
          >
            {renameWorkspace.isPending || renameLead.isPending ? (
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ) : (
              <Save className="h-3.5 w-3.5" />
            )}{" "}
