...
function AcceptInvitePage() {
  ...
  const [status, setStatus] = useState<
    "loading" | "no-token" | "invalid" | "form" | "accepting" | "done"
  >("loading");
  const [invite, setInvite] = useState<InviteInfo | null>(null);
  const [sessionUser, setSessionUser] = useState<{ id: string; email: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        .from("invites")
        .select("id,email,workspace_id,workspace_role,expires_at,workspaces(name)")
        .eq("token", token)
        .is("accepted_at", null)
        .maybeSingle();
      ...
      setInvite({
        ...
        workspace_name:
          (data as unknown as { workspaces: { name: string } | null }).workspaces
            ?.name ?? null,
  ...
  if (status === "loading") return <Center><Loader2 className="h-6 w-6 animate-spin text-primary" /></Center>;
      ...
      <Frame>
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            It may have already been used or expired. Ask Dream Wave Media to
            send you a new one.
          </p>
          <Link
            to="/auth"
            className="mt-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-glow)] hover:brightness-110"
          >
            Go to sign in
          </Link>
        </div>
